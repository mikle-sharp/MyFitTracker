...
import { Exercise, WorkoutSet, isAbsExercise, getExerciseType, EXERCISE_TYPE_COLORS, WorkoutType } from '@/lib/types';
...
export function ExerciseCard({ 
  ...
  const handleAddSet = () => {
    ...
    addSet(workoutId, exercise.id, reps, weight, time > 0 ? time : undefined, isWarmup && exercise.sets.length === 0 ? true : undefined);
    ...
    <>
      <motion.div
        ...
        className={cn(
          ...
          "border-l-4",
          exerciseColors.border.replace('/50', '')
        )}
      >
        {/* Header */}
          ...
          {isAddingSet ? (
            <div className="space-y-2 mt-4 pt-4 border-t border-zinc-700">
              ...
              <div className="flex gap-2 flex-wrap items-center">
                {/* Чекбокс разминки - только для первого подхода */}
              ...
              {/* Input fields */}
              <div className="flex items-center gap-2 flex-wrap">
                <div className="w-7 h-7 rounded-lg bg-emerald-600/20 flex items-center justify-center text-xs font-medium text-emerald-400 shrink-0">
            ...
            <Button
              ...
              className="w-full mt-3 border-dashed border-zinc-600 text-zinc-400 hover:text-white hover:border-emerald-500 hover:bg-emerald-500/10"
            >
              Добавить подход
            </Button>
          )}
